...
namespace K4os.Compression.LZ4.Engine
{
    internal unsafe class LZ4_64 : LZ4_xx
    {
        ...
        public static void LZ4_resetStream(LZ4_stream_t* state) =>
            LZ4Mem.Zero((byte*)state, sizeof(LZ4_stream_t));
        ...
        protected static uint LZ4_count(byte* pIn, byte* pMatch, byte* pInLimit)
        {
            ...
            if (pIn < pInLimit - 3 && LZ4Mem.Peek32(pMatch) == LZ4Mem.Peek32(pIn))
            {
                pIn += 4;
                pMatch += 4;
            }

            if (pIn < pInLimit - 1 && LZ4Mem.Peek16(pMatch) == LZ4Mem.Peek16(pIn))
            {
                pIn += 2;
                pMatch += 2;
            }
