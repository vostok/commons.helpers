...
namespace Vostok.Commons.Helpers.Tests.Observable
{
    ...
    {
        [Test]
        public void CatchErrors_should_complete_when_source_observable_completes_without_error()
        {
            ...
            HealingObservable.CatchErrors(() => subject, 100.Milliseconds())
                .Count().Wait().Should().Be(2);
        }
        
        [Test]
        public void CatchErrors_should_restart_source_observable_on_error()
        {
            var observables = new[]
            {
                CreateObservable(false, 1, 2, 3),
                CreateObservable<int>(false),
                CreateObservable(false, 4, 5),
                CreateObservable(true, 6)
            };
            var index = 0;

            HealingObservable.CatchErrors(() => observables[index++], 100.Milliseconds())
